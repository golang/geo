...
// testRectCentroidSplitting recursively verifies that when a rectangle is split into two pieces, the
...
func testRectCentroidSplitting(t *testing.T, r Rect, leftSplits int) {
	...
	const epsilon = 2e-15
	if got := r.Centroid().Sub(child0.Centroid().Vector).Sub(child1.Centroid().Vector).Norm(); got > epsilon {
		t.Errorf("%v.Centroid() - %v.Centroid() - %v.Centroid = %v, want <= %v", r, child0, child1, got, epsilon)
	}
	if leftSplits > 0 {
