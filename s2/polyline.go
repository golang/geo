...
func (p *Polyline) findEndVertex(tolerance s1.Angle, index int) int {
	...
	// vectors to a one-dimensional "ray angle" in the range (-Pi,
	// Pi].  The angle of a direction vector is computed by
	...
	// represent it as an S1Interval, i.e. an interval on the unit
	// circle.
	origin := (*p)[index]
	...
	for index++; index < len(*p); index++ {
		...
		if distance > s1.Degree*90 && lastDistance > 0 {
			break
		}
...
// Return a polyline containing a subsequence of vertices such that
// the polyline connecting these vertices is never further than
// "tolerance" from the original polyline.  The first and last
...
//    vertices than the Douglas-Peucker algorithm with the same
//    tolerance.
//
//  - The output is *parametrically* equivalent to the original
//    polyline to within the given tolerance.  For example, if a
//    polyline backtracks on itself and then proceeds onwards, the
//    backtracking will be preserved (to within the given tolerance).
//    This is different than the Douglas-Peucker algorithm which only
//    guarantees geometric equivalence.
func (p *Polyline) SubsampleVertices(tolerance s1.Angle) *Polyline {
	result := make(Polyline, 0)
	...
	for index := 0; index+1 < len(*p); {
		nextIndex := p.findEndVertex(clampedTolerance, index)
		// Don't create duplicate adjacent vertices.
		if (*p)[nextIndex] != (*p)[index] {
			result = append(result, (*p)[nextIndex])
		}
		index = nextIndex
	}
